        ...
        if dir.is_dir() {
            for entry in std::fs::read_dir(dir)? {
                ...
                if path.is_dir() {
                    ...
                    if name.len() >= 10 {
                        if let Ok(date) = chrono::NaiveDate::parse_from_str(&name[0..10], "%Y-%m-%d") {
                            if date < cutoff_date {
...
pub fn create_snapshot_if_needed(
    ...
    if !force {
        if let Some(latest_path) = snapshots.last() {
            if let Some(name) = latest_path.file_name().and_then(|n| n.to_str()) {
                if name.len() >= 19 {
                    if let Ok(last_time) = chrono::NaiveDateTime::parse_from_str(&name[0..19], "%Y-%m-%d_%H-%M-%S") {
                        if let Some(last_dt) = chrono::Local.from_local_datetime(&last_time).single() {
    ...
    let original_filename = note_path.file_name()
