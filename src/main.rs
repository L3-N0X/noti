...
/// Holds the state of the single window once it has been built, so that a
/// second launch (for example "Open with…" while noti is already running) can
/// reuse it instead of creating another one.
type SharedUi = Rc<RefCell<Option<Rc<RefCell<AppState>>>>>;
...
fn main() -> glib::ExitCode {
    ...
    let ui: SharedUi = Rc::new(RefCell::new(None));

    let ui_activate = ui.clone();
    application.connect_activate(move |app| {
        present(app, &ui_activate, None);
    });

    // Triggered by "Open with…", `noti file.md`, or `gio open`.
    let ui_open = ui.clone();
    application.connect_open(move |app, files, _hint| {
        let mut paths = files.iter().filter_map(|file| file.path());
        let first = paths.next();
        let ignored = paths.count();

        present(app, &ui_open, first);

        if ignored > 0 {
            if let Some(state) = ui_open.borrow().clone() {
                if let Ok(state) = state.try_borrow() {
                    state.show_message(&format!(
                        "noti opens one file at a time — ignored {} more",
                        ignored
                    ));
                }
            }
...
fn present(app: &adw::Application, ui: &SharedUi, file: Option<PathBuf>) {
    if let Some(app_state) = ui.borrow().clone() {
        if let Some(path) = file {
            if let Ok(mut state) = app_state.try_borrow_mut() {
                if let Err(e) = state.open_external_path(&path) {
                    state.show_error(&format!("Failed to open {}: {}", path.display(), e));
                }
            }
        }
    ...
    let app_state = build_ui(app, file);
    *ui.borrow_mut() = Some(app_state);
}

fn build_ui(app: &adw::Application, initial_file: Option<PathBuf>) -> Rc<RefCell<AppState>> {
    ...
    key_controller.connect_key_pressed(move |_, keyval, _, state| {
        ...
        if has_ctrl {
            if keyval_lower == Key::p && !has_shift {
                ...
                glib::Propagation::Stop
            } else if keyval_lower == Key::s && !has_shift {
                AppState::execute_command_static(app_state_key.clone(), "Save now");
                glib::Propagation::Stop
    ...
    if let Ok(mut state) = app_state.try_borrow_mut() {
        ...
        if let Err(e) = state.load_initial_note(initial_file) {
